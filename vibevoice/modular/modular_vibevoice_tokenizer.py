...
class TokenizerEncoder(nn.Module):
    ...
    def __init__(self, config):
        ...
        # 强制 device="cpu": transformers 5.x 的 from_pretrained 会在 meta device 上下文里
        # 构建模块, torch.linspace 会落到 meta tensor, .item() 报 'cannot be called on meta'.
...
# transformers 5.3.0 会在类定义时自动登记这两个 config, 模块底部再显式 register 就重复,
# 无 exist_ok 时抛 'VibeVoiceAcousticTokenizerConfig is already used'. 幂等注册即可。

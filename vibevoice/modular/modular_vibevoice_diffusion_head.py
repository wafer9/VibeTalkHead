...
class VibeVoiceDiffusionHead(PreTrainedModel):
    ...
    def __init__(
        ...
        # Dimension of the incoming condition (LM hidden states). May differ from
        # the head's internal hidden_size, so project from cond_input_dim -> cond_dim.
        cond_input_dim = getattr(config, "cond_dim", None) or config.hidden_size

        self.noisy_images_proj = nn.Linear(latent_size, config.hidden_size, bias=False)
        self.cond_proj = nn.Linear(cond_input_dim, self.cond_dim, bias=False)

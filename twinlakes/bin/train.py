...
def main():
    ...
    model.lm.get_input_embeddings().weight.requires_grad = False
    if getattr(model.lm, 'lm_head', None) is not None:
        # tie 时与 embed_tokens 同一张量, 冻结上一行即已生效; 非 tie 时这里再兜底冻结。
        model.lm.lm_head.weight.requires_grad = False
    ...
    train_data_loader = DataLoader(train_dataset, **loader_kwargs)
    cv_data_loader = DataLoader(cv_dataset, **loader_kwargs)
    ...
    if rank == 0:
        ...
        num_params = sum(p.numel() for p in model.diffusion_head.parameters() if p.requires_grad)
        print('the number of model requires_grad params : {:,d}'.format(num_params))
    ...
    # 所有可训练模块 (lm/comp/acoustic_connector/video_dit) 每步都参与 forward,
    # 故默认关闭 find_unused_parameters (省显存/提速, 且与 gradient checkpointing 兼容)。
    ...
    base_lr = optim_conf.pop('lr')
    lm_lr = configs.get('lm_lr', base_lr)
    lm_param_ids = {id(p) for p in model.module.lm.parameters()}
    lm_params = [p for p in model.parameters() if p.requires_grad and id(p) in lm_param_ids]
    local_params = [p for p in model.parameters() if p.requires_grad and id(p) not in lm_param_ids]
    optimizer = optim.Adam(
        [
            {'params': lm_params, 'lr': lm_lr},
            {'params': local_params, 'lr': base_lr},
        ],
        **optim_conf,
    )
    ...
    # executor 存 checkpoint 时读 configs['model_dir'],但它只来自命令行 args,
    # 需显式塞进 configs 否则 KeyError。

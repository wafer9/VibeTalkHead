    ...
    def train(self, model, optimizer, scheduler, train_data_loader,
        ...
        with model_context():
            for batch_idx, batch_dict in enumerate(train_data_loader):
                ...
                video_latents = batch_dict['video_latents'].to(self.device, dtype=dtype)
                video_latent_lengths = batch_dict['video_latent_lengths'].to(device=self.device)

                input_ids = batch_dict['input_ids'].to(self.device)
                labels = batch_dict['label_ids'].to(self.device)


                info_dict["bs"] = wavs.size(0)
                info_dict['tokens'] = wavs.size(1)
